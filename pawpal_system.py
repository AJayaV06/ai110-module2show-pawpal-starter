...
class Scheduler:
    ...
    def generate_daily_plan(self) -> List[Task]:
        all_tasks = [t for t in self.retrieve_tasks() if not t.completed and t.is_repeat_due(self.date)]
        all_tasks = [t for t in all_tasks if t.validate()]
        ...
        for i in range(len(tasks) - 1):
            ...
            if current_end and nxt.scheduled_start:
                if datetime.combine(self.date, nxt.scheduled_start) < datetime.combine(self.date, current_end):
                    conflicts.append(
                        f"{current.title} ({current.scheduled_start}-{current_end}) overlaps {nxt.title} ({nxt.scheduled_start}-{nxt.get_end_time()})"
                    )
